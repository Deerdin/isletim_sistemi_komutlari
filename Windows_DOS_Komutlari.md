# ${{\color{purple}W}}\{{\color{green}i}}\{{\color{pink}n}}\{{\color{yellow}d}}\{{\color{red}o}}\{{\color{orange}w}}\{{\color{brown}s}}\ \ \{{\color{blue}DOS \ komutları}}$
## ${{\color{purple} 1. }}\{{\color{cyan}Komut}}$ ---> `hostname` Komutu bilgisayarın adını gösterir.
```shell
hostname 
```
![SS1](https://github.com/user-attachments/assets/e445135b-ca9f-459f-8388-3944d9f0bc5e)
## ${{\color{purple} 2. }}\{{\color{cyan}Komut}}$  ---> `dir` Komutu dizinin yolunu gösterir.
```shell
dir  
```
![SS2](https://github.com/user-attachments/assets/8a62e3b6-5efb-4f4f-b23d-0f674ed0305f)
## ${{\color{purple} 3. }}\{{\color{cyan}Komut}}$  ---> `cd` Komutu belirtilen dizine geçer.
```shell
cd C:\Users
```
```shell
cd C:\Users\erdin
```
```shell
cd C:\Users\erdin\Desktop
```
![SS3](https://github.com/user-attachments/assets/3f778c40-d0a4-4d52-aeb1-eac44a095eb9)
## ${{\color{purple} 4. }}\{{\color{cyan}Komut}}$  ---> `mkdir` Komutu yeni bir klasör oluşturur.
```shell
mkdir yeni_klasor  
```
![SS4](https://github.com/user-attachments/assets/1a717e0f-5d2d-4628-8632-de92a413da4d)
## ${{\color{purple} 5. }}\{{\color{cyan}Komut}}$  ---> `del` Komutu bir dosya siler.
```shell
del yeni_klasor
```
![SS5](https://github.com/user-attachments/assets/49c6b66f-4b80-4fc2-8ec5-311b37332dd5)
## ${{\color{purple} 6. }}\{{\color{cyan}Komut}}$  ---> `tree` Komutu dizin yapısını ağaç şeklinde gösterir.
```shell
tree
```
![SS6](https://github.com/user-attachments/assets/c97f9800-50e2-4cad-842d-c68b31eeb999)
## ${{\color{purple} 7. }}\{{\color{cyan}Komut}}$  ---> `ren` Komutu bir dizinin adını değiştirir.
```shell
ren eski_isim yeni_isim
```
![SS7](https://github.com/user-attachments/assets/8f4e54da-c850-470f-bd11-46418c33849e)
## ${{\color{purple} 8. }}\{{\color{cyan}Komut}}$  ---> `type` Komutu metin belgesinin içeriğini görüntüler.
```shell
type yeni.txt
```
![SS8](https://github.com/user-attachments/assets/5a0058dd-b4a8-4e25-ab7c-587c11bcf316)
## ${{\color{purple} 9. }}\{{\color{cyan}Komut}}$  ---> `exit` Komutu terminali kapatır.
```shell
exit
```
![SS9](https://github.com/user-attachments/assets/b0e9e5eb-fe01-4d8b-b4d8-a4de509b9dc3)
## ${{\color{purple} 10. }}\{{\color{cyan}Komut}}$  ---> `resmon` Komutu kaynak izleyicisinin penceresini açar.
```shell
resmon
```
![SS10](https://github.com/user-attachments/assets/4d6f35ce-236e-4c9f-b7a3-ab37a1ba3d66)
## ${{\color{purple} 11. }}\{{\color{cyan}Komut}}$  ---> `msinfo32` Komutu sistem bilgilerini gösterir. 
```shell
msinfo32
```
![SS11](https://github.com/user-attachments/assets/51a9c439-f5a9-422d-8f17-f705f8aa6314)
## ${{\color{purple} 12. }}\{{\color{cyan}Komut}}$  ---> `winver` Komutu  bir pencere açarak Windows sürüm bilgilerini gösterir.
```shell
winver
```
![SS12](https://github.com/user-attachments/assets/571ab10d-a45a-4b20-bda5-d7237c45e822)
## ${{\color{purple} 13. }}\{{\color{cyan}Komut}}$  ---> `move` Komutu dosyaları taşımada kullanılır.
```shell
move beni_tasi buraya_tasi
```
![SS13](https://github.com/user-attachments/assets/af6095c7-0520-4631-acab-385d9bc75c46)
## ${{\color{purple} 14. }}\{{\color{cyan}Komut}}$  ---> `whoami` Komutu geçerli kullanıcıyı gösterir.
```shell
whoami  
```
![SS14](https://github.com/user-attachments/assets/0660ed9e-0a3d-41f3-af9e-0a8e430f94bf)
## ${{\color{purple} 15. }}\{{\color{cyan}Komut}}$  ---> `date` Komutu sistem saatini gösterir.
```shell
date 
```
![SS15](https://github.com/user-attachments/assets/2bb16ec4-a3e2-4646-b684-7859fdef1474)
## ${{\color{purple} 16. }}\{{\color{cyan}Komut}}$  ---> `ping` Komutu ağ bağlantılarının doğruluğunu ve ağ cihazlarının erişilebilirliğini test etmek için kullanılır.
```shell
ping google.com 
```
![SS16](https://github.com/user-attachments/assets/17420b29-856e-4b75-a4e0-9c6492188ff2)
## ${{\color{purple} 17. }}\{{\color{cyan}Komut}}$  ---> `echo` Komutu terminale yazdırmak istenileni yazdırır.
```shell
echo `Merhaba Dünya`
```
![SS17](https://github.com/user-attachments/assets/07b0a9a0-03de-4241-8920-28f8eb46c0f3)
## ${{\color{purple} 18. }}\{{\color{cyan}Komut}}$  ---> `cls` Komutu terminali temizler.
```shell
cls
```
![SS18](https://github.com/user-attachments/assets/4a0c537d-b9a1-4492-b3d1-44b9a40c5844)
## ${{\color{purple} 19. }}\{{\color{cyan}Komut}}$  ---> `history` Komutu terminalde yazdırdığınız komutların geçmişini sunar (sonuna yazılan sayı kaçıncı sırada olduğunu gösterir).
```shell
history 5
```
![SS19](https://github.com/user-attachments/assets/873c040e-33c1-460a-a5b9-379b298126c7)
## ${{\color{purple} 20. }}\{{\color{cyan}Komut}}$  ---> `copy` Komutu dosyaları belirtilen dizine kopyalar.
```shell
copy beni_kopyala buraya_kopyala 
```
![SS20](https://github.com/user-attachments/assets/fb1cea01-bd62-40bf-92f8-aad8cbb0f663)
## ${{\color{purple} 21. }}\{{\color{cyan}Komut}}$  ---> `shutdown` Komutu 30 saniye sonra bilgisayarı kapatır.
```shell
shutdown /s /t 30
```
![SS21](https://github.com/user-attachments/assets/db4d6b17-91ca-40a8-bfe4-51965ffcf3c8)
## ${{\color{purple} EXTRA }}\{{\color{cyan}Komut}}$  ---> `powershell` üzerinden çalıştırılabilen bir pencere açmamıza yarayan güzel bir kod.
```shell
$isim=read-host -prompt "Lütfen adınızı giriniz"
$mesaj="Sen ne iyi bir insansın "+$isim
$wshell = New-Object -ComObject Wscript.Shell
$wshell.Popup($Mesaj,0,"Günün Mesaji",0x1)
```
![SSEXTRA](https://github.com/user-attachments/assets/e531ee87-a533-4a5b-b7cb-31fc797de87a)
## ${{\color{purple} EXTRA }}\{{\color{cyan}Komut}}$  ---> `curl parrot.live` cmd'yi yönetici olarak çalıştırıp komutu yazdığımızmda güzel renkli bir papağanın giff görüntüsünü sağlayan kod.
```shell
curl parrot.live
```
![SSEXTRA2](https://github.com/user-attachments/assets/cf33f775-8c87-4286-89be-c6c5886afee7)






















